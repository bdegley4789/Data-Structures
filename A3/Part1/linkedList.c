...
void _initList (struct linkedList *lst) {
  ...
  lst->lastLink->next = 0; //Not circular
  lst->firstLink->prev = 0; //So its null
  lst->size = 0;
}
...
void _addLinkBefore(struct linkedList *lst, struct DLink *l, TYPE v)
{
   ...
   assert(lst != NULL);
   assert(l != NULL);
   struct DLink *temp = (struct DLink*)malloc(sizeof(struct DLink)); //new link
   temp->value = v; 
   temp->next = l; /*point at desired link to go before*/
   temp->prev = l->prev; /*link before before*/
   l->prev->next = temp; /*at new added link*/
   l->prev = temp; /*before link*/
   lst->size++;
}
