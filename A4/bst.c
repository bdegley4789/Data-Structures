...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    temp->val = val; //Create new node with value
    if (cur == 0) { //If spot is empty
      return temp;
    } else {
       if (compare(val,cur->val)==1) { //Move to correct order
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    while (temp != 0) {
      if (compare(val,temp->val)==-1) {
         temp = temp->left; //Compares to find location
      } else if (compare(temp->val,val)==0) {
         return 1; //If found
      } else {
         temp = temp->right;
      }
...
TYPE _leftMost(struct Node *cur)
{
    ...
    assert(cur != NULL);
    if (cur->left == 0) {
      return cur->val; //Once no more lefts
    } else {
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
   if (compare(cur->val,val)==0) {
      if (cur->right == 0) { //right most on left side
         return cur->left; 
      } else {
         cur = _removeLeftMost(cur->right);
         cur->val = _leftMost(cur->right); //leftmost right side
         return cur;
      }
   }
   struct Node *temp = (struct Node *) malloc(sizeof(struct Node));
   ...
   if (compare(temp->val,val)!=0) { 
      if (compare(val,cur->val)==1) { //finding node
