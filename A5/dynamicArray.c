...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
  ...
  addDynArr(heap,val);
    int stop = 0;
  while(position != 0 && stop != 1) {
      top = (position-1)/2;
      if(compare(getDynArr(heap,position),getDynArr(heap,top))>=0){ /*move until position*/
         stop = 1;
      } else {
          swapDynArr(heap,top,position);
      }
      position = top;
   }
}
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
  ...
  int right = 2 * pos + 2;
  int left = 2 * pos + 1;
  int small = -1; //avoid unused warning
  if (right < max) {
...
void removeMinHeap(DynArr *heap, comparator compare)
{
  /* FIXME: Write this */
  assert(heap->size != 0);
  putDynArr(heap,0,getDynArr(heap,heap->size-1));
  removeAtDynArr(heap,heap->size-1);
  _adjustHeap(heap,heap->size-1,0,compare);
}
