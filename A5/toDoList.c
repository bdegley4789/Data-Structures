...
{
    /*FIXME: write this*/
    struct Task * leftTemp = (struct Task*)left;
    struct Task * rightTemp = (struct Task*)right;
    if (leftTemp->priority < rightTemp->priority) {
      return -1;
    } 
    if (leftTemp->priority > rightTemp->priority) {
      return 1;
...
TaskP createTask (int priority, char *desc)
{
  ...
  strcpy(temp->description,desc); /*for pointer*/
  return temp;
}
